...
def make_score_matrix(align_profiles, del_cost, ins_cost,
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            scores[i, j] = scores[j, i] = needleman_wunsch_score_only(
                align_profiles[i], align_profiles[j],
                del_cost, ins_cost, match_cost, mismatch_cost
            )
    return scores
...
def multiple_sequence_alignment(seqs, del_cost, ins_cost,
                                match_cost, mismatch_cost):
    n = len(seqs)
    align_profiles = [AlignmentItem.from_seq(seq, i)
                      for i, seq in enumerate(seqs)]
    scores = make_score_matrix(align_profiles,
                               del_cost, ins_cost,
                               match_cost, mismatch_cost)
    for i in range(n - 2):
